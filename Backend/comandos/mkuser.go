...
func Mkuser(user string, pwd string, grp string) {
	if user == "" || pwd == "" || grp == "" {
		...
		respuesta += "\nERROR MKUSER: NO SE INGRESO TODOS LOS PARAMETROS OBLIGATORIOS"
		return
	}
	if usuario_activo.Uid != "1" { //solo root
		//fmt.Println("ERROR MKUSER: SOLO EL USUARIO ROOT PUEDE REALIZAR ESTA OPERACION")
		respuesta += "\nERROR MKUSER: SOLO EL USUARIO ROOT PUEDE REALIZAR ESTA OPERACION"
		return
	}
	...
	for _, registro := range registros { //UID [0], TIPO [1], GRUPO [2], USUARIO [3], CONTRASEÑA [4]
		if len(registro) == 5 {
			if registro[3] == user {
				...
				respuesta += "\nERROR MKUSER: EL USUARIO " + user + " YA EXISTE"
				return
			}
			cont++
		}
